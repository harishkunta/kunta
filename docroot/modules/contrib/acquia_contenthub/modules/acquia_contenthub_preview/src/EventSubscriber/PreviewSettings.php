...
class PreviewSettings implements EventSubscriberInterface {
  ...
  public static function getSubscribedEvents() {
    $events[AcquiaContentHubEvents::GET_SETTINGS][] = ['onGetSettings', 10001];
    return $events;
  }
  ...
  public function onGetSettings(AcquiaContentHubSettingsEvent $event) {
    ...
    $settings = new Settings($config->get('client_name'), $config->get('origin'), $config->get('api_key'), $config->get('secret_key'), $hostname, $config->get('shared_secret'), $config->get('webhook') ?? []);
    if ($settings) {
      $event->setSettings($settings);
      $event->setProvider('preview_site');
